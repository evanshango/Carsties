...
const AuctionForm: FC<Props> = ({auction}: Props) => {
    ...
    const onSubmit = async (data: FieldValues): Promise<void> => {
        try {
            ...
            await router.push(`/auctions/details/${id}`)
        } catch (error: any) {
    ...
    return (
        <form className='flex flex-col mt-3' onSubmit={handleSubmit(onSubmit)}>
                ...
                <>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Input label={'Reserve Price (enter 0 if no reserve)'}
                               name={'reservePrice'} control={control} rules={{required: 'Reserve price is required'}}
                               type='number'
                        />
                        <DateInput label={'Auction end date/time'} name={'auctionEnd'} control={control}
                                   dateFormat='dd MMMM yyyy h:mm a'
                                   showTimeSelect rules={{required: 'Auction end date is required'}}
                        />
                    </div>
                </>
            )}
            <div className="flex justify-between">
                ...
                <Button isProcessing={isSubmitting} type='submit' color='success' outline
